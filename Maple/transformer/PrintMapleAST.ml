...
and print_memberfuncs (oc: out_channel) (mf: memberfuncs) =
  match mf with
  | MFnil -> ()
  | MFcons (id, tsl, ts, fal, mf, loc) ->
     fprintf oc "\n\t&%s %a (%a) %a,%a" id print_func_attr_list fal print_typespeclist tsl print_typespec ts print_memberfuncs mf
...
let rec print_statement (oc: out_channel) (s: statement) =
  ...
  | S_brtrue (lbl, e, loc) ->
     fprintf oc "brtrue %a (%a)" print_label lbl print_expr e
  | S_brfalse (lbl, e, loc) ->
     fprintf oc "brfalse %a (%a)" print_label lbl print_expr e
  | S_goto (lbl, loc) ->
  ...
  | S_virtualcallassigned (id, el, l, loc) ->
     fprintf oc "virtualcallassigned &%s (%a) { %a }" id print_exprlist el print_assign_list l
  | S_interfacecallassigned (id, el, l, loc) ->
     fprintf oc "interfacecallassigned &%s (%a) { %a }" id print_exprlist el print_assign_list l
  | S_javatry (ll, s, loc) ->
...
let print_local_declaration (oc: out_channel) (ld: local_declaration) =
  match ld with
  | LD_var (id, vd, loc) -> fprintf oc "var %%%s %a" id print_var_def vd
  | LD_type (id, t, loc) -> fprintf oc "type %%%s %a" id print_coq_type t
  | LD_preg (i, pt, loc) -> fprintf oc "reg %%%a %a" print_intInfo i print_prim_type pt

let rec print_local_declaration_list (oc: out_channel) (ldl: local_declaration list) =
  match ldl with
  | [] -> ()
  | ld :: ldl ->
     fprintf oc "%a\n%a" print_local_declaration ld print_local_declaration_list ldl

let print_function_body (oc: out_channel) (fb: function_body) =
  fprintf oc "{\n%a\n%a\n}" print_local_declaration_list fb.fun_localdecl print_statement fb.fun_statement
...
let print_global_declaration (oc: out_channel) (gd: global_declaration) =
  match gd with
  | GD_var (id, vd, loc) ->
     fprintf oc "var $%s %a" id print_var_def vd
  | GD_type (id, t, loc) ->
     fprintf oc "type $%s %a" id print_coq_type t
  | GD_func (id, f, loc) ->
     fprintf oc "func &%s %a" id print_coq_function f
  | GD_javaclass (id1, id2, cal, loc) ->
     fprintf oc "javaclass $%s <$%s> %a" id1 id2 print_class_attr_list cal
  | GD_javainterface (id1, id2, cal, loc) ->
     fprintf oc "javainterface $%s <$%s> %a" id1 id2 print_class_attr_list cal
  | GD_entryfunc (id, loc) ->
     fprintf oc "entryfunc &%s" id

let rec print_global_declaration_list (oc: out_channel) (gdl: global_declaration list) =
  match gdl with
  | [] -> ()
  | gd :: gdl ->
     fprintf oc "%a\n%a" print_global_declaration gd print_global_declaration_list gdl

let print_program = print_global_declaration_list
